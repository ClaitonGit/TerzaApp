...
class CalSapataCorridaAgua extends StatefulWidget {
  const CalSapataCorridaAgua({super.key});


  @override
  State<CalSapataCorridaAgua> createState() => _CalSapataAguaState();
}

class _CalSapataAguaState extends State<CalSapataCorridaAgua> {
  ...
  void resultQuadradaAgua(){
    ...
    setState(() {
      ...
      if (nv < cota){
        double result = (coesao*nc)+((cota*peso+(nv*(peso-10)))*nq)+(peso*(largura/2)*ny);
        infoText = "Sapata Corrida: (${result})";
        ...
        double result = (coesao*nc)+(cota*peso*nq)+((v2*peso+v3*(peso-10))/bu)*(largura/2)*ny;
        infoText = "Sapata Corrida: (${result})";
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text("Sapata Corrida com água"),
        centerTitle: true,
        actions: [
          IconButton(
